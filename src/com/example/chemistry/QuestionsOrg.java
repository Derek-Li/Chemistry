...
public class QuestionsOrg extends Activity implements OnCheckedChangeListener,
		OnClickListener {
	TextView question;
	Button submit;
	...
	private void setRootQuestion() {
		if (questionId == null) {
			...
			questionId = "organic" + questionId;
			
			counter = Integer.parseInt(questionId.replaceAll("\\D",""));
		} else {
			if (counter <= getResources().getStringArray(R.array.organicquestions).length) 

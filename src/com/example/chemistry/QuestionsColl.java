...
public class QuestionsColl extends Activity implements OnCheckedChangeListener,
		OnClickListener {
	TextView question;
	Button submit;
	...
	private void setRootQuestion() {
		if (questionId == null) {
			...
			questionId = "coll" + questionId;
			
			counter = Integer.parseInt(questionId.replaceAll("\\D",""));
		} else {
			if (counter <= getResources().getStringArray(R.array.collquestions).length) 
